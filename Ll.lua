...
local Window = Library:AddWindow("PHANTOM HUB 👻 | By Ti", {
    main_color = Color3.fromRGB(130, 0, 255), -- color morado phantom, si lo quieres rojo pon 255,0,0
    min_size = Vector2.new(600, 600),
    can_resize = false,
})

-- HACE TODA LA LETRA GRUESA Y BLANCA
task.spawn(function()
    repeat task.wait() until Window and Window.Container
    for _,v in pairs(Window.Container:GetDescendants()) do
        if v:IsA("TextLabel") or v:IsA("TextButton") or v:IsA("TextBox") then
            v.TextColor3 = Color3.fromRGB(255, 255, 255)
            v.Font = Enum.Font.GothamBold
            v.TextSize = 15
            if v.TextStrokeTransparency then
